...
impl LysisExecProfile {
    fn from_env<F: FieldBackend>(program: &Program<F>) -> Option<Self> {
    ...
    fn record(&mut self, instr: &crate::program::Instr, template_id: Option<u16>, ran: u64) {
    ...
    fn should_abort(&self, ran: u64) -> bool {
        self.abort_after
            .map(|abort_after| ran >= abort_after)
            .unwrap_or(false)
    }

    fn print(&self, ran: u64) {
