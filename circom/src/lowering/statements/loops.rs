...
fn body_has_local_var_array_indexed_writes(stmts: &[Stmt], env: &LoweringEnv) -> bool {
    stmts
        .iter()
...
fn body_has_indexed_assign_shape(stmts: &[Stmt]) -> bool {
    stmts.iter().any(stmt_has_indexed_assign_shape)
}

fn stmt_has_indexed_assign_shape(stmt: &Stmt) -> bool {
    match stmt {
        Stmt::Substitution {
            target,
            op: AssignOp::Assign,
...
fn stmt_has_local_var_array_indexed_write(stmt: &Stmt, env: &LoweringEnv) -> bool {
    match stmt {
        Stmt::Substitution {
            target,
            op: AssignOp::Assign,
...
fn is_local_var_array_indexed(expr: &Expr, env: &LoweringEnv) -> bool {
    let mut cursor = expr;
...
fn body_has_local_var_array_indexed_reads(stmts: &[Stmt], env: &LoweringEnv) -> bool {
    stmts
        .iter()
        .any(|s| stmt_has_local_var_array_indexed_read(s, env))
}

fn stmt_has_local_var_array_indexed_read(stmt: &Stmt, env: &LoweringEnv) -> bool {
    match stmt {
        Stmt::Substitution { target, value, .. } => {
...
fn expr_reads_local_var_array(expr: &Expr, env: &LoweringEnv) -> bool {
    match expr {
        Expr::Index { object, index, .. } => {
