...
//!
//! Evaluates Circom expressions and statements at compile time using 256-bit
//! `BigVal` values. Supports arithmetic, comparisons, control flow (if/else,
//! for, while), function calls, and array construction.
...
pub(super) fn eval_stmt(
    ...
    match stmt {
        Stmt::VarDecl { names, init, .. } => {
            let val = match init {
                Some(expr) => eval_expr(expr, vars, functions, depth)?,
                None => BigVal::ZERO,
            };
            for name in names {
        ...
        Stmt::Substitution {
            ...
            if let Expr::Ident { name, .. } = target {
                let val = eval_expr(value, vars, functions, depth)?;
                vars.insert(name.clone(), val);
                Some(StmtResult::Continue)
            } else {
                None
        ...
        Stmt::CompoundAssign {
            ...
            if let Expr::Ident { name, .. } = target {
                ...
                let rhs = eval_expr(value, vars, functions, depth)?;
        ...
        Stmt::Expr { expr, .. } => {
            ...
            eval_expr(expr, vars, functions, depth)?;
            Some(StmtResult::Continue)
        }
        ...
        Stmt::While {
            ...
            for _ in 0..MAX_EVAL_ITERATIONS {
                let cond = eval_expr(condition, vars, functions, depth)?;
                if cond.is_zero() {
                    return Some(StmtResult::Continue);
                }
...
pub(super) fn eval_expr_value(
    ...
    match expr {
        ...
        Expr::Index { object, index, .. } => {
            ...
            let idx = eval_expr(index, vars, functions, depth)?;
            let idx_usize = idx.to_u64()? as usize;
            arr.index(idx_usize).cloned()
        }
        Expr::Number { .. } | Expr::HexNumber { .. } => {
            if let Some(v) = eval_expr(expr, vars, functions, depth) {
                Some(EvalValue::Scalar(v))
            } else {
                Some(EvalValue::Expr(Box::new(expr.clone())))
            }
        }
        _ => eval_expr(expr, vars, functions, depth).map(EvalValue::Scalar),
    }
}
...
pub fn eval_expr(
    ...
    match expr {
        ...
        Expr::BinOp { op, lhs, rhs, .. } => {
            let l = eval_expr(lhs, vars, functions, depth)?;
            let r = eval_expr(rhs, vars, functions, depth)?;
            eval_binop(l, *op, r)
        }
        Expr::UnaryOp { op, operand, .. } => {
            let val = eval_expr(operand, vars, functions, depth)?;
            match op {
        ...
        Expr::Ternary {
            ...
            let cond = eval_expr(condition, vars, functions, depth)?;
            if !cond.is_zero() {
                eval_expr(if_true, vars, functions, depth)
            } else {
                eval_expr(if_false, vars, functions, depth)
            }
        }
        Expr::Call { callee, args, .. } => {
            let name = extract_ident_name(callee)?;
