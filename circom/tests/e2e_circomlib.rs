...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M constraints), kept ignored as a long-running integration probe that currently does not complete on commodity hardware: under an 11 GiB virtual-memory sandbox it aborts inside execute after ~6 min wall-clock when a ~1.1 GB single allocation hits the cap with peak RSS at ~10 GB (see the RSS-pressure paragraph below). It compiles cheaply (well under a gigabyte). The wide-output WitnessCall lysis frame overflow is closed on both the outputs-only path (pinned by secp256k1_addunequal_loop_nested_lysis_frame_fit) and the cold-input path (pinned by witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic); the lysis const-pool index width is closed (pinned by load_const_idx_survives_when_const_pool_exceeds_u16_ceiling); the lysis program-global heap-slot index width is closed (pinned by heap_slots_stay_distinct_when_spill_count_exceeds_u16_ceiling and heap_slot_above_u16_ceiling_round_trips_the_right_value); the lysis executor tail-eliminates the walker's linear template-instantiation chain (pinned by tail_chain_runs_in_constant_stack_past_max_call_depth, non_tail_call_chain_still_overflows_under_low_max_call_depth, and rule11_tail_chain_does_not_count_toward_depth) so a circuit whose split chain exceeds the call-stack depth no longer overflows. the lysis instruction budget does not apply to the trusted compiler-internal replay of walker-generated bytecode, whose termination is guaranteed structurally rather than by an instruction cap (pinned by for_internal_replay_lifts_the_instruction_ceiling). The probe is wired to the chunk-draining entry point (instantiate_lysis_drain_with_outputs) so each lysis-emitted chunk is fed straight into R1CSCompiler::compile_instructions_streaming at chunk-seal time and the chunk's backing allocation drops before the next chunk fills; the streaming variant preserves the R1CS operand-lookup caches across chunk-seal boundaries (pinned by compile_instructions_streaming_resolves_operands_across_batches) so cross-chunk operand references — a Mul whose operand SsaVar was first emitted in an earlier sealed chunk — resolve correctly. Peak resident state inside execute stays at `interner dedup state + 1 chunk + growing R1CS state` instead of growing with the total emission count — the streaming_chunks Vec no longer accumulates, the IR-side `Vec<Instruction<F>>` handoff is never materialised, and the Lysis-side instruction stream's single doubling allocation also stays closed. The lysis emission-counter index width is closed: NodeId and EffectId are 64-bit non-zero handles, the IR-side SsaVar is a 64-bit newtype, and the lysis-to-R1CS bridge cast widens to match, so a multi-billion-emission program no longer exhausts the 32-bit handle space (pinned by fresh_node_id_advances_past_u32_max). The next wall on commodity hardware is RSS pressure inside execute: the per-program state that the streaming compile preserves across chunk-seal boundaries (such as the SsaVar-keyed R1CS operand cache used to resolve cross-chunk operand references) grows with the total emitted-operand count, and at >1B emitted operands a single rehash allocation can exceed the available virtual-memory budget. Closing that wall requires either a chunk-keyed eviction policy on the preserved caches or a more memory-compact representation of the LinearCombination values they hold; pinning which cache dominates is a measurement task ahead of the fix. Run with --ignored only."]
