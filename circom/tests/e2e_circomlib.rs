...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M constraints). Under an 11 GiB virtual-memory sandbox it aborts inside `instantiate_lysis_drain_with_outputs` during the setup phase — before any chunk drains and before `[instantiate+drain]` prints — when the lysis interner state plus the lysis-side scratch buffers push virtual address space past the cap. Closing this requires reducing the peak resident state of the lysis interner setup (deferred-string pool, interner tables, ConstPool allocation), not further work on the per-chunk emission path or the constraint-system structures: the per-instruction enum layout is already at its current minimum (`Box<WitnessCallBody>` keeps both `Instruction` and `InstructionKind` at 56 bytes, pinned by `instruction_size_pinned` and `instruction_kind_size_pinned`), and any constraint-system attack would help only after the setup phase completes. The wide-output `WitnessCall` lysis frame overflow is closed on both the outputs-only path (pinned by `secp256k1_addunequal_loop_nested_lysis_frame_fit`) and the cold-input path (pinned by `witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic`); the lysis const-pool and program-global heap-slot index widths are closed (pinned by `load_const_idx_survives_when_const_pool_exceeds_u16_ceiling`, `heap_slots_stay_distinct_when_spill_count_exceeds_u16_ceiling`, `heap_slot_above_u16_ceiling_round_trips_the_right_value`); the lysis executor tail-eliminates the walker's linear template-instantiation chain (pinned by `tail_chain_runs_in_constant_stack_past_max_call_depth`); the lysis emission-counter widening to 64-bit non-zero handles is pinned by `fresh_node_id_advances_past_u32_max`. The probe is wired to the chunk-draining entry point so each lysis chunk drains into `R1CSCompiler::compile_instructions_streaming` at seal time (cross-chunk operand refs pinned by `compile_instructions_streaming_resolves_operands_across_batches`), but on this sandbox the setup-phase ceiling fires first. Run with --ignored only."]
