...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M constraints), kept ignored as a slow integration probe. It compiles cheaply (well under a gigabyte). The wide-output WitnessCall lysis frame overflow is closed on both the outputs-only path (pinned by secp256k1_addunequal_loop_nested_lysis_frame_fit) and the cold-input path (pinned by witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic); the lysis const-pool index width is closed (pinned by load_const_idx_survives_when_const_pool_exceeds_u16_ceiling); the lysis program-global heap-slot index width is closed (pinned by heap_slots_stay_distinct_when_spill_count_exceeds_u16_ceiling and heap_slot_above_u16_ceiling_round_trips_the_right_value); the lysis executor tail-eliminates the walker's linear template-instantiation chain (pinned by tail_chain_runs_in_constant_stack_past_max_call_depth, non_tail_call_chain_still_overflows_under_low_max_call_depth, and rule11_tail_chain_does_not_count_toward_depth) so a circuit whose split chain exceeds the call-stack depth no longer overflows. the lysis instruction budget does not apply to the trusted compiler-internal replay of walker-generated bytecode, whose termination is guaranteed structurally rather than by an instruction cap (pinned by for_internal_replay_lifts_the_instruction_ceiling). The probe now executes the full multi-pass instantiation of the genuine 1.5M-constraint circuit and is wired to the streaming entry point (instantiate_lysis_sink_with_outputs + R1CSCompiler::compile_instructions) so the IR-side Vec<Instruction<F>> handoff never materialises. The Lysis-side instruction stream's single doubling allocation is also closed: when LYSIS_STREAMING_WINDOW=N and LYSIS_CHUNKED_OUTPUT=1 are set, emission is chunked (one million slots per chunk, ~72 MB on Bn254Fr) and the constraint backend drains chunks as they are produced so no multi-gigabyte single allocation appears. The remaining wall on commodity hardware is total in-process live content: the lysis executor and the R1CS backend run sequentially in the same process, so the chunk producer's tail-end interner state coexists with growing R1CS state and the still-in-flight emission stream. Closing it is a separate arc — lifting the executor → backend boundary to a producer/consumer pair so each chunk drops before the next one is built. Run with --ignored only."]
fn ecdsa_verify_boss_fight() {
    ...
    let bundle = result
        .prove_ir
        .instantiate_lysis_sink_with_outputs(&fe_captures, &result.output_names)
        .unwrap_or_else(|e| panic!("ECDSAVerify instantiate failed: {e}"));
    eprintln!("[ECDSAVerify] [instantiate]  {:?}", t1.elapsed());

    let t3 = Instant::now();
    let mut rc = R1CSCompiler::<Bn254Fr>::new();
    // Chunked-drain consumer: when the sink was built with chunked
    // streaming output (`LYSIS_CHUNKED_OUTPUT=1`), each chunk's backing
    // allocation drops as the iterator crosses chunk boundaries so the
    // resident footprint shrinks monotonically while compile_instructions
    // runs. Falls back to the flat-Vec path when the env var is unset.
    rc.compile_instructions(ir_forge::lysis_sink_chunked_instruction_stream(bundle.sink))
        .expect("ECDSAVerify R1CS compile");
    let pre_o1 = rc.cs.num_constraints();
    eprintln!(
