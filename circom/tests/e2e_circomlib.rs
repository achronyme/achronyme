...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M post-O1 constraints). Under an 11 GiB virtual-memory sandbox it aborts inside `instantiate_lysis_drain_with_outputs` after successfully draining 10s of chunks of 1M IR instructions each — the failing alloc is small (~hundreds of bytes), the process is at ~10 GB RSS, and `[instantiate+drain]` does not print because execute() never returns. Setup leaves RSS at ~3 GB (instantiate-extended on the lean path skips the `var_names`/`var_spans`/`input_spans` HashMaps because the chunk-drain caller would drop them anyway; only `var_types` is built since the ternary type-propagation reads it during emission), then each chunk drain adds several hundred MB on average to the R1CSCompiler's accumulated state (`cs.constraints`, `lc_map`, `witness_ops`, `range_bounds`, `divmod_cache`, `proven_boolean`, `bool_enforced`). `constraint_origins` is skipped here via the lean compiler constructor — the inspector / CLI provenance readers are not exercised by this prove path. The gap to closure is roughly several more chunks of R1CS-side state. Closing this requires compacting the per-chunk-accumulated state inside `R1CSCompiler::compile_instructions_streaming`, NOT further work on the per-instruction enum layout (already at its minimum with `Box<WitnessCallBody>` keeping both `Instruction` and `InstructionKind` at 56 bytes, pinned by `instruction_size_pinned` and `instruction_kind_size_pinned`). The lysis-side closures remain pinned by `secp256k1_addunequal_loop_nested_lysis_frame_fit`, `witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic`, `load_const_idx_survives_when_const_pool_exceeds_u16_ceiling`, `heap_slots_stay_distinct_when_spill_count_exceeds_u16_ceiling`, `heap_slot_above_u16_ceiling_round_trips_the_right_value`, `tail_chain_runs_in_constant_stack_past_max_call_depth`, `fresh_node_id_advances_past_u32_max`, and the cross-chunk operand refs by `compile_instructions_streaming_resolves_operands_across_batches`. The lean-instantiate contract (chunk-drain entry never populates `var_names`/`var_spans`/`input_spans`) is pinned by `lean_instantiate_extended_skips_name_and_span_maps` and `lean_instantiate_with_outputs_extended_skips_name_and_span_maps`. The lean-compiler contract (`new_lean` never populates `constraint_origins`) is pinned by `lean_compiler_skips_constraint_origins` and `lean_compiler_matches_eager_on_everything_except_origins`. Run with --ignored only."]
