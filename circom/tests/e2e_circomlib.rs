...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M constraints), kept ignored as a slow integration probe. It compiles cheaply (well under a gigabyte). The wide-output WitnessCall lysis frame overflow is closed on both the outputs-only path (pinned by secp256k1_addunequal_loop_nested_lysis_frame_fit) and the cold-input path (pinned by witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic); the lysis const-pool index width is closed (pinned by load_const_idx_survives_when_const_pool_exceeds_u16_ceiling) so a >65535-entry pool no longer truncates an opcode index. The probe now advances deep into lysis instantiate of the genuine 1.5M-constraint circuit; its current wall is a StoreHeap heap-slot-bound violation at instantiate, handled as a separate arc (the program-global heap is a different index space than the const pool). Run with --ignored only."]
