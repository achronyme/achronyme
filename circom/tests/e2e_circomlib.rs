...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M post-O1 constraints). Under an 11 GiB virtual-memory sandbox the chunk-drain consumer seals 22 chunks of ~365K witness ops each (cs.num_constraints ≈ 11.6M, witness_ops.len ≈ 8.3M at the end of chunk 22) before the next chunk's R1CS state push triggers a HashMap rehash request of 1 GiB that the sandbox refuses, and the process aborts. Setup leaves RSS at ~3 GB (instantiate-extended on the lean path skips the `var_names`/`var_spans`/`input_spans` HashMaps because the chunk-drain caller would drop them anyway; only `var_types` is built since the ternary type-propagation reads it during emission), then each chunk drain adds ~250-700 MB on average to the R1CSCompiler's accumulated state (`cs.constraints`, `lc_map`, `witness_ops`, `range_bounds`, `divmod_cache`, `proven_boolean`, `bool_enforced`). `constraint_origins` is skipped here via the lean compiler constructor — the inspector / CLI provenance readers are not exercised by this prove path. The R1CS backend also content-interns Artik bytecode payloads, so the ~99% byte-weighted dedup across the few unique witness-program templates does not cumulatively occupy `witness_ops`. The remaining wall is the `lc_map` HashMap's capacity-doubling allocation request hitting the sandbox ceiling. Closing it requires either compacting `lc_map` itself or rebalancing per-chunk accumulation. The per-instruction enum layout is already at its minimum (`Box<WitnessCallBody>` keeps both `Instruction` and `InstructionKind` at 56 bytes, pinned by `instruction_size_pinned` and `instruction_kind_size_pinned`). The lysis-side closures remain pinned by `secp256k1_addunequal_loop_nested_lysis_frame_fit`, `witness_call_routes_to_heap_when_cold_inputs_would_overflow_classic`, `load_const_idx_survives_when_const_pool_exceeds_u16_ceiling`, `heap_slots_stay_distinct_when_spill_count_exceeds_u16_ceiling`, `heap_slot_above_u16_ceiling_round_trips_the_right_value`, `tail_chain_runs_in_constant_stack_past_max_call_depth`, `fresh_node_id_advances_past_u32_max`, and the cross-chunk operand refs by `compile_instructions_streaming_resolves_operands_across_batches`. The lean-instantiate contract (chunk-drain entry never populates `var_names`/`var_spans`/`input_spans`) is pinned by `lean_instantiate_extended_skips_name_and_span_maps` and `lean_instantiate_with_outputs_extended_skips_name_and_span_maps`. The lean-compiler contract (`new_lean` never populates `constraint_origins`) is pinned by `lean_compiler_skips_constraint_origins` and `lean_compiler_matches_eager_on_everything_except_origins`. The Artik content-interning contract is pinned by `artik_intern_shares_arc_for_identical_payloads`, `artik_intern_keeps_distinct_arcs_for_different_payloads`, and `artik_intern_survives_across_streaming_batches` (the last one guards the cross-chunk path this fixture uses). Run with --ignored only."]
fn ecdsa_verify_boss_fight() {
    ...
    let t1 = Instant::now();
    let mut rc = R1CSCompiler::<Bn254Fr>::new_lean();
