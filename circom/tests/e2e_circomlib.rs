...
fn ecdsa_verify_boss_fight() {
    ...
    //
    // Compile errors from `compile_instructions` cannot be returned
    // through the executor's sink trait (the IrSink methods are
    // infallible), so the consumer absorbs them into a side-channel
    // that this fixture checks once the executor returns.
    let t1 = Instant::now();
    let mut rc = R1CSCompiler::<Bn254Fr>::new();
    ...
    let mut consumer = |chunk: Vec<lysis::InstructionKind<Bn254Fr>>| {
        ...
        if let Err(e) = rc.compile_instructions(stream) {
