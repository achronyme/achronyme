...
#[test]
#[ignore = "ECDSAVerify(64, 4) is the heaviest probe in this file (>1.5M constraints). The witness lift covers the full secp256k1 bigint chain — `getProperRepresentation`, `isNegative`'s runtime `cond ? a : b`, `long_div` and the rest all lift to Artik subprograms — so compilation proceeds past the witness lift. The per-iteration `all_constants` materialization in the unrolled scalar-multiplication ladder (both component-call scalar-arg resolution and component-array target resolution) has been eliminated: CPU is now near-linear per node (~9x faster at 7.6M nodes, no super-linear term). The sole remaining blocker is memory: a single monotonically-growing `Vec<CircuitNode>` accumulator, fed by non-shared recursive component-body inlining concatenation across the ~256-step ladder, reaches a ~1.9 GB realloc and OOMs under an 8 GB cap before the compile finishes. That is a separate structural arc (component-body sharing / promotion). Run with --ignored only."]
