...
pub fn native_len(vm: &mut VM, args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    if val.is_string() {
        ...
        let s = vm.heap.get_string(handle).ok_or(RuntimeError::Unknown("String not found".into()))?;
        Ok(Value::number(s.len() as f64))
    } else if val.is_list() {
        let handle = val.as_handle().unwrap();
        let l = vm.heap.get_list(handle).ok_or(RuntimeError::Unknown("List not found".into()))?;
        Ok(Value::number(l.len() as f64))
    } else if val.is_map() {
        // Placeholder until Map is fully exposed in Heap
         Err(RuntimeError::TypeMismatch("Map length not yet supported via native".into()))
    } else {
...
pub fn native_typeof(vm: &mut VM, args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArityMismatch("typeof() takes exactly 1 argument".into()));
    }
    let val = &args[0];
    let type_name = if val.is_number() { "Number" }
    else if val.is_string() { "String" }
    else if val.is_bool() { "Bool" }
    else if val.is_nil() { "Nil" }
    else if val.is_list() { "List" }
    else if val.is_map() { "Map" }
    else if val.is_function() { "Function" } // Script function
    else if val.is_native() { "Native" }
    else if val.is_complex() { "Complex" }
    else { "Unknown" };
...
pub fn native_assert(_vm: &mut VM, args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArityMismatch("assert() takes exactly 1 argument".into()));
    }
