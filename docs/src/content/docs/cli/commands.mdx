...
| Flag | Description |
|------|-------------|
| `--error-format <fmt>` | Diagnostic output format: `human` (default), `json`, or `short`. See [Diagnostics & Warnings](/language/diagnostics/#output-formats) |
...
See [Project Configuration](/cli/project-config) for the `achronyme.toml` reference.
...
| Flag | Description |
|------|-------------|
| `--stress-gc` | Run GC on every allocation (for testing). See [Stress GC Mode](/architecture/memory-and-gc/#stress-gc-mode) |
| `--max-heap <size>` | Set maximum heap size (e.g., `256M`, `1G`, `512K`). Raises `HeapLimitExceeded` if exceeded |
| `--gc-stats` | Print GC statistics to stderr after execution. See [GC Statistics](/architecture/memory-and-gc/#gc-statistics) |
