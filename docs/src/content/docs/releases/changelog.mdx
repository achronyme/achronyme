...
- Parameters declare visibility: `name: Public` or `name: Witness Field[N]`
...
- Anonymous `prove(name: Public)` syntax unchanged
...
- Array annotations (`Field[N]`, `Bool[N]`) propagated to `prove(name: Public)` captures — enables ProveIR to decompose array captures into individual elements.

### Prove block array captures

Array variables from VM scope are now automatically captured by `prove(name: Public)` blocks — no type annotations required:

```ach
let path = [voter1, n1]
let indices = [0p0, 0p0]

prove(merkle_root: Public, nullifier: Public) {
...
- **`prove(name: Public)`** — Concise prove block syntax with witness auto-inference. Only public inputs need to be declared; all other outer-scope variables are automatically captured as private witnesses.
