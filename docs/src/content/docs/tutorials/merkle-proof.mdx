...
```ach
// Depth 1 (2 leaves)
circuit merkle_d1(root: Public, leaf: Witness, path: Witness Field[1], indices: Witness Field[1]) { ... }

// Depth 5 (32 leaves)
circuit merkle_d5(root: Public, leaf: Witness, path: Witness Field[5], indices: Witness Field[5]) { ... }

// Depth 20 (~1M leaves)
circuit merkle_d20(root: Public, leaf: Witness, path: Witness Field[20], indices: Witness Field[20]) { ... }
```
...
For explicit type safety, add type annotations to the circuit parameters:

```ach
circuit merkle_typed(root: Public Field, leaf: Witness Field, path: Witness Field[3], indices: Witness Bool[3]) {
    merkle_verify(root, leaf, path, indices)
}
```
