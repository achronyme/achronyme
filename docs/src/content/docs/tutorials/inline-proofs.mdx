...
prove {
    witness a     // 10 becomes field(10)
    witness b     // 20 becomes field(20)
    public sum    // 30 becomes field(30)
    assert_eq(a + b, sum)
}
```

Outside prove blocks, mixing Int and Field raises a `TypeMismatch` error. Use `field()` for explicit conversion.
