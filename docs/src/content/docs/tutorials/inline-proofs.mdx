...
The explicit `witness`/`public` declaration syntax inside the block body is also supported:
...
Both syntaxes produce identical circuits. The param-style `prove(hash: Public)` form is preferred and more concise; the explicit form gives full control. You cannot mix both in the same block.
