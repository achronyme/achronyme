...
3. Marca `product` como entrada **pública** (declarada como `Public`)
4. Genera una prueba Groth16 en tiempo de ejecución
5. Devuelve un `ProofObject`

## Auto-Inferencia de Testigos

Con la sintaxis `prove(...)`, solo declaras qué variables son públicas — todo lo demás se infiere automáticamente como testigo:
...
- `hash` está declarado como `Public` → **entrada pública** (visible para el verificador)
...
La sintaxis explícita `witness`/`public` dentro del bloque también está soportada:
...
Ambas sintaxis producen circuitos idénticos. La forma `prove(hash: Public)` es más concisa; la forma explícita da control total. No se pueden mezclar ambas en el mismo bloque.
