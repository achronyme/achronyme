...
3. Marca `product` como entrada **pública** (declarada en `public: [...]`)
4. Genera una prueba Groth16 en tiempo de ejecución
5. Devuelve un `ProofObject`

## Auto-Inferencia de Testigos

Con la sintaxis `prove(public: [...])`, solo declaras qué variables son públicas — todo lo demás se infiere automáticamente como testigo:
...
- `hash` está en `public: [...]` → **entrada pública** (visible para el verificador)
...
Ambas sintaxis producen circuitos idénticos. La forma `prove(public: [...])` es más concisa; la forma explícita da control total. No se pueden mezclar ambas en el mismo bloque.
