...
La visibilidad y el tipo se combinan en los parámetros del circuito:

```ach
circuit example(root: Public Field, flag: Witness Bool, secret: Witness Field, path: Witness Field[3], indices: Witness Bool[3]) {
    // ...
}
```

Sin tipo explícito, las entradas usan solo la visibilidad:

```ach
circuit example(root: Public, secret: Witness) {
    // ...
}
```
...
```ach
circuit example(root: Public Field, leaf: Witness, path: Witness Field[3], indices: Witness Field[3]) {
    // ...
}
```
...
```ach
circuit example(a: Witness Field, b: Witness Field) {
...
```ach
circuit example(a: Witness Field) {
...
```ach
circuit example(x: Witness, y: Witness) {
...
```ach
circuit example(flag: Witness Bool) {
...
- **Tipado como `Field`**: Error. Un valor explícitamente tipado como `Field` no puede estrecharse a `Bool`:
  ```
  circuit example(x: Witness Field) {
...
```ach
// Sin anotación de tipo: cada mux agrega 1 verificación booleana para cond
circuit example(cond: Witness, a: Witness, b: Witness, c: Witness, d: Witness) {
...
// Con anotación Bool: 1 verificación en la declaración, usos posteriores omiten
circuit example(cond: Witness Bool, a: Witness, b: Witness, c: Witness, d: Witness) {
    // cond: 1 restricción (verificación RangeCheck)
...
| `b: Witness Bool` → usado en `mux(b, ...)` | **Ahorra 1 por uso** — `mux` omite verificación booleana en `b` |
...
```ach
circuit merkle(root: Public Field, leaf: Witness Field, path: Witness Field[3], indices: Witness Bool[3]) {
...
circuit hash_check(expected: Public Field, a: Witness Field, b: Witness Field, c: Witness Field) {
...
|--------|---------|-------|
| Public tipado | `x: Public Field` | Entrada pública anotada |
...
| Sin tipo (predeterminado) | `secret: Witness` | Mismo comportamiento que antes |
...
- **Ayudar a bloques prove** — las anotaciones de array como `Field[N]` habilitan la captura automática de arrays en bloques `prove(...)`
