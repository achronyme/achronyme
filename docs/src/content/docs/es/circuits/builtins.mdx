...
```ach
circuit check(x: Witness, y: Witness) {
    assert(x < y)
}
```
...
```ach
circuit hash_chain(expected: Public, a: Witness, b: Witness, c: Witness) {
...
```ach
circuit range(x: Witness, y: Witness) {
...
```ach
circuit merkle(root: Public, leaf: Witness, path: Witness Field[1], indices: Witness Field[1]) {
...
```ach
circuit example(vals: Witness Field[3]) {
    let n = len(vals)
    assert_eq(n, 3)
}
```
