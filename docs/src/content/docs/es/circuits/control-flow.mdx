...
```ach
circuit example(out: Public, a: Witness, b: Witness) {
    let product = a * b
...
```ach
circuit matrix_check(matrix: Witness Field[9]) {
    for i in 0..3 {
        for j in 0..3 {
...
```ach
circuit select(cond: Witness, a: Witness, b: Witness) {
...
```ach
circuit example(a: Witness, b: Witness) {
