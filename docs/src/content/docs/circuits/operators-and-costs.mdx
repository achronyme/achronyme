...
```ach
circuit powers(x2: Public, x3: Public, x4: Public, x: Witness) {
    assert_eq(x ^ 2, x2)
    assert_eq(x ^ 3, x3)
    assert_eq(x ^ 4, x4)
}
...
```ach
circuit logic(x: Witness, y: Witness) {
    let both = (x < y) && (y > x)
    assert(both)
