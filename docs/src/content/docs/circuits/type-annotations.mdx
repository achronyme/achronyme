...
Annotating a witness as `Bool` at the declaration site tells the compiler it can skip the boolean enforcement constraint when the variable is used in boolean context later. This saves **1 constraint per use** in operations like `mux`, `&&`, `||`, `!`, and `assert`.
...
- Constants `0` and `1`
- Comparison results (`==`, `!=`, `<`, `<=`, `>`, `>=`)
- `RangeCheck(x, 1)` results (including those emitted by `: Bool` enforcement)
- `Assert` operands and results
- Variables annotated with `IrType::Bool` (from `witness b: Bool` declarations)
...
| Scenario | Cost |
|----------|------|
| `witness b: Bool` → used in `mux(b, ...)` | **Saves 1** — declaration marks `b` as boolean, `mux` skips enforcement |
...
| Typed witness | `witness flag: Bool` | Saves 1 boolean enforcement constraint per use |
