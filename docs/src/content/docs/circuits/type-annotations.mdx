...
In circuit parameters, the visibility (`Public` or `Witness`) comes first, followed by an optional type:
...
Without type annotations, inputs default to untyped — same behavior as before:
...
```ach
circuit merkle(root: Public Field, leaf: Witness, path: Witness Field[3], indices: Witness) {
    // ...
}
```
...
circuit example(cond: Witness Bool, a: Witness, b: Witness, c: Witness, d: Witness) {
    // cond: Witness Bool → 1 constraint (RangeCheck enforcement)
...
| `b: Witness Bool` → used in `mux(b, ...)` | **Saves 1 per use** — `mux` skips boolean enforcement on `b` |
...
```ach
circuit hash_chain_check(expected: Public Field, a: Witness Field, b: Witness Field, c: Witness Field) {
...
|--------|---------|-------|
| Typed public | `x: Public Field` | Annotated public input |
...
- **Help prove blocks** — array annotations like `Field[N]` enable automatic array capture in `prove(...)` blocks
