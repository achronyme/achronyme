...
The `prove` block captures variables from scope, compiles the body as a circuit via ProveIR at compile time, and generates a proof at runtime. Variables in `public: [...]` are public inputs; all other outer-scope variables are auto-inferred as private witnesses.
