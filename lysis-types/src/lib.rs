...
//! types that cross the boundary between Lysis and its consumers (for
//! now `ir` via `lysis_bridge`; post-P7 also `ir-forge`). By keeping
//! these types outside of `lysis` proper, consumers can depend on
//! `lysis-types` without pulling in the bytecode encoder, interner, or
//! VM executor.
//!
//! ## Scope
//!
//! What lives here: [`NodeId`], [`Visibility`], [`InstructionKind<F>`]
//! — the minimal set of types that producers of interned Lysis nodes
//! and materializers of those nodes both need to speak.
//!
