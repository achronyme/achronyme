...
fn import_basic_function_call() {
    ...
    let result = cli::commands::run::run_file(&fixture("main_vm.ach"), false, None, "r1cs", EF);
...
fn import_constants_access() {
    let result =
        cli::commands::run::run_file(&fixture("test_constants.ach"), false, None, "r1cs", EF);
    assert!(result.is_ok(), "run_file failed: {:?}", result.err());
}
...
fn import_internal_helper_function() {
    ...
    let result =
        cli::commands::run::run_file(&fixture("test_internal.ach"), false, None, "r1cs", EF);
    assert!(result.is_ok(), "run_file failed: {:?}", result.err());
}
...
fn import_transitive() {
    ...
    let result =
        cli::commands::run::run_file(&fixture("transitive/c.ach"), false, None, "r1cs", EF);
    assert!(result.is_ok(), "run_file failed: {:?}", result.err());
}

#[test]
fn import_circular_detected() {
    let result = cli::commands::run::run_file(&fixture("circular_a.ach"), false, None, "r1cs", EF);
...
fn import_module_not_found() {
    let result =
        cli::commands::run::run_file(&fixture("test_not_found.ach"), false, None, "r1cs", EF);
    assert!(result.is_err());
    let err = format!("{}", result.unwrap_err());
    assert!(
...
fn import_no_exports_module() {
    ...
    let result =
        cli::commands::run::run_file(&fixture("test_no_exports.ach"), false, None, "r1cs", EF);
    assert!(result.is_ok(), "run_file failed: {:?}", result.err());
}
