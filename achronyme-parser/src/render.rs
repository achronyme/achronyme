...
mod tests {
    ...
    fn multi_line_error() {
        let source = "fn foo() {\n    let x = 1\n    let y = 2\n}";
        let diag = Diagnostic::error("missing semicolon", SpanRange::new(15, 35, 2, 5, 3, 14));
        let output = render_plain(source, &diag);
        assert!(output.contains("2 |     let x = 1"));
        assert!(output.contains("3 |     let y = 2"));
    }
    ...
    fn secondary_label() {
        ...
        let diag = Diagnostic::error(
            ...
            SpanRange::new(14, 15, 2, 5, 2, 6),
        )
        .with_label(SpanRange::new(4, 5, 1, 5, 1, 6), "first defined here");
        let output = render_plain(source, &diag);
    ...
    #[test]
    fn warning_output() {
        let source = "let _x = 42";
        let diag = Diagnostic::warning("unused variable `_x`", SpanRange::new(4, 6, 1, 5, 1, 7))
            .with_code("W001");
        let output = render_plain(source, &diag);
        assert!(output.contains("warning[W001]: unused variable `_x`"));
    }

    #[test]
    fn color_output_contains_ansi() {
        let source = "let x = 1";
        let diag = Diagnostic::error("test", SpanRange::new(0, 3, 1, 1, 1, 4));
        let output = DiagnosticRenderer::new(source, ColorMode::Always).render(&diag);
    ...
    fn point_span_single_caret() {
        ...
        let output = render_plain(source, &diag);
        assert!(output.contains("^"));
        // Should have exactly one caret at position
