...
use lysis::{
    execute, expected_family, ChunkDrainingSink, InstructionKind, InterningSink, IrSink,
    LysisConfig, NodeId,
};
...
fn direct_plain_drain_enabled() -> bool {
    std::env::var("ACH_LYSIS_DIRECT_PLAIN_DRAIN")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn direct_plain_validate_enabled() -> bool {
    std::env::var("ACH_LYSIS_DIRECT_PLAIN_VALIDATE")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn trace_first_plain_forward_ref<F: FieldBackend>(body: &[ExtendedInstruction<F>]) {
...
fn drain_plain_extended_chunks_interned<F: FieldBackend>(
...
mod tests {
    ...
    fn direct_plain_drain_chunks_plain_body_without_walker() {
        ...
        let total = super::drain_plain_extended_chunks_interned(body, 8, 2, &mut |chunk| {
            chunks.push(chunk)
        });
